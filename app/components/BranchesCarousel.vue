...
// Branches carousel (Figma 965:15404) — full-bleed track of uniform 600px branch
// cards that steps one at a time; the active card is pinned to the first-item
// slot with blurred peeks on either side. Same mechanics as HomeNewsCarousel
// (measure → clamp travel → drag/auto-advance), only the card and metrics differ.
...
const GAP = 32
const CARD_W_MAX = 400 // design width; the card never grows past it
...
// The design's 600px card is far wider than a phone, and the root is
// `overflow-hidden` — a fixed card would be permanently clipped at every index
// (there is no size ramp here to recover it). Shrink it to fit instead. Card
// HEIGHT needs no clamping: the photo is a fixed aspect box and both text lines
// are `truncate`, so every card stays the same height at any width.
...
// runs while the carousel is actually on screen (`onScreen`), so it never burns
// cycles — or silently skips past cards — while parked elsewhere on the page.
// Hover pauses so a card can be read, a backgrounded tab suspends it, and any
// manual step restarts the countdown. Skipped for prefers-reduced-motion.
const AUTO_MS = 5000
...
// as a normal click that opens the card's Google Maps link; only a real drag
// suppresses that trailing click and steps by however many card-widths were
// dragged.
...
function onPointerUp(e: PointerEvent) {
  ...
  if (!wasDragging) {
    startAuto() // a tap — let the click open the map link
    return
  }
...
// The peek box is the design's full 404px (965:16099), but the blur only ramps in
// over its last ~3/4. The first 26% ≈ 105px covers the tail of the last
// fully-visible card, which the design render keeps crisp — a ramp starting at
// x=0 hazes that card's right edge instead.
...
<template>
  <div>
    <div
      ...
      @dragstart.prevent
    >
      <!-- No track height: every card is a fixed-aspect photo over a
           fixed-line-count body, so the row is uniform on its own. The 20px
           card shadow would be clipped by the root's overflow-hidden, so the
           track carries 24px of vertical padding to let it breathe — the
           section subtracts that from its own padding to keep the design's
           120px above the cards and 64px below them. -->
      <div
        ref="trackEl"
      ...
      <!-- Blurred peeks — previous cards on the left, upcoming on the right.
           404px at lg matches the design's blur rectangle (965:16099); the
           feathered mask replaces Figma's hard-edged rect, which would otherwise
           put a visible seam down the middle of the last fully-visible card. -->
      <div
      ...
      <!-- Floating nav (Figma 965:16100): a frosted arrow button straddling the
           content column's edge — the design centres it exactly on x=1560 in a
           1920 frame, i.e. one --carousel-edge in from the right, so the offset
           is that edge less the button's 22px half-width. Wrapper handles
           position + show/hide so IconButton stays untouched; pointer-events
           only on the 44px circle so the rest of the peek still accepts drag.
           `.stop` keeps a button press from also starting a track drag. -->
      <div
        v-show="overflowsLeft"
    ...
    <!-- Controls (Figma 965:16078): 44px row aligned to the card column. mt-10 +
         the track's py-6 adds up to the design's 64px gap under the card row.
         gap-20 is the design's 82.8px inset before the progress line, rounded to
         the nearest token — the 82.8 is slop from a hand-drawn line, not intent. -->
